...
const availables = require('../models/availableitem');
...
router.post('/middleTermInsert', (req, res, next) => {
    ...
    MiddleTermItem.addMiddleTermItem(newMiddleTermItem, (err, middleTermItem) => {
        if (err) {
            ...
            availables.addAvailableItem(newMiddleTermItem, (err, middleTermItem) => {
                if (err) {
                    res.json({ success: false, msg: 'Failed to insert into available item' });
                    console.log(err);
                } else {
                    res.json({ success: true, msg: 'Item inserted to available' });
                }
            });
...
router.post('/updateMiddle', function(request, response) {
    ...
    MiddleTermItem.findOneAndUpdate(request.params.itemName, {
            $set: {
                ...
                warrantyType: request.body.warrantyType,
                warrantyQuantity: request.body.warrantyQuantity,
                referenceId: request.body.referenceId,
                itemWarehouse: request.body.itemWarehouse,
...
router.post('/updateShort', function(request, response) {
    ...
    ShortTermItem.findOneAndUpdate(request.params.itemName, {
            $set: {
                itemName: request.body.itemName,
                itemType: request.body.itemType,
                itemQuntity: request.body.itemQuntity,
                itemWarehouse: request.body.itemWarehouse,
                itemDescription: request.body.itemDescription
            }
        }, {
